...
// Vorgabe des Betriebs (FamilyQuan Bistro), Arbeitszeit:
//   Jeden Tag (7 Tage die Woche)  12:00–22:00 DURCHGEHEND – EIN Block, kein
//   geteilter Dienst. Die gesetzliche Pause (60 min bei langen Diensten, siehe
//   calculatePause) liegt innerhalb des Dienstes, nicht als Ladenschließung.
//   Kein Ruhetag: der Chef steht die ganze Woche in der Küche.
const DURCHGEHEND: DayBlocks = [w(12 * 60, 22 * 60)];

export const DEFAULT_WORK_HOURS: WorkHoursConfig = {
  perWeekday: {
    monday: DURCHGEHEND.map((b) => ({ ...b })),
    tuesday: DURCHGEHEND.map((b) => ({ ...b })),
    wednesday: DURCHGEHEND.map((b) => ({ ...b })),
    thursday: DURCHGEHEND.map((b) => ({ ...b })),
    friday: DURCHGEHEND.map((b) => ({ ...b })),
    saturday: DURCHGEHEND.map((b) => ({ ...b })),
    sunday: DURCHGEHEND.map((b) => ({ ...b })),
  },
  // Feiertage: der Laden ist OFFEN, durchgehend – wie jeder andere Tag.
  holiday: DURCHGEHEND.map((b) => ({ ...b })),
  closedWeekdays: {
    // FamilyQuan hat KEINEN Ruhetag – 7 Tage offen.
    monday: false,
    tuesday: false,
    wednesday: false,
    thursday: false,
