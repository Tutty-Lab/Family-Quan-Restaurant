...
// Jeder Bereich (Küche/Service) bekommt ein eigenes Kürzel, damit die
// Schicht-IDs über die getrennt geplanten Bereiche hinweg eindeutig bleiben.
let shiftIdTag = "";
function nextShiftId(): string {
  shiftIdCounter += 1;
  return shiftIdTag ? `gen-${shiftIdTag}-${shiftIdCounter}` : `gen-${shiftIdCounter}`;
}
...
/**
 * Feste durchgehende Küchen-Schicht des Chefs an JEDEM offenen Tag.
 *
 * Der Chef (FamilyQuan) steht die ganze Woche in der Küche, über das komplette
 * Öffnungsfenster (12:00–22:00). Damit ist die Küche zu jeder offenen Minute
 * grundbesetzt; die übrigen Küchenkräfte kommen zusätzlich in der Stoßzeit
 * dazu. Er läuft NICHT über die normale Planungsrunde (kein Monats-Soll steuert
 * ihn) und wird von den Reparaturläufen nicht angefasst – er wird ganz am Ende
 * belegt. Die Sechs-Tage-Regel gilt für ihn nicht: er arbeitet sieben Tage.
 * Eingetragener Urlaub spart einen Tag aus.
 */
function placeOwnerShifts(state: SchedulerState, owners: Employee[]): void {
  for (const emp of owners) {
    for (const iso of state.dates) {
      const day = state.dayOf(iso);
      if (day.closed) continue;
      if (!mayWorkOn(emp, iso)) continue; // Urlaub
      const frame = frameOf(day.blocks);
      const presence = frame.endMinutes - frame.startMinutes;
      if (presence <= 0) continue;
      let paid = presence;
      for (let i = 0; i < 3; i++) paid = presence - calculatePause(paid);
      const pause = presence - paid;
      if (paid <= 0) continue;
      const shift: Shift = {
        id: nextShiftId(),
        employeeId: emp.id,
        date: iso,
        startMinutes: frame.startMinutes,
        endMinutes: frame.endMinutes,
        pauseMinutes: pause,
        paidMinutes: paid,
        shiftType: "EARLY",
        generated: true,
      ...
      applyShift(state, shift);
      state.remaining.set(emp.id, (state.remaining.get(emp.id) ?? 0) - paid);
    }
  }
}
...
export function generateSchedule(input: GenerateInput): Shift[] {
  ...
  const allEmployees = input.employees.map((e) => ({
    ...e,
    targetMinutes: monthlyTargetMinutes(e, openDays),
  }));

  // FamilyQuan ist in zwei Bereiche geteilt (Küche/Service), die UNABHÄNGIG
  // voneinander besetzt sein müssen. Deshalb wird jeder Bereich für sich
  // geplant: die Nachfrage, die Stoßzeit-Abdeckung und der Sechs-Tage-Rhythmus
  // gelten dann je Bereich. Kräfte ohne Bereich (workRole leer) bilden eine
  // eigene Gruppe – so bleibt der Scheduler für Filialen ohne Bereiche
  // unverändert.
  const order: string[] = [];
  const groups = new Map<string, Employee[]>();
  for (const e of allEmployees) {
    const key = e.workRole ?? "NONE";
    if (!groups.has(key)) {
      groups.set(key, []);
      order.push(key);
    }
...
/**
 * Plant genau EINEN Bereich (alle Kräfte einer workRole) für sich: Nachfrage,
 * Stoßzeit und Sechs-Tage-Regel gelten nur innerhalb dieser Gruppe. Gibt die
 * Schichten dieses Bereichs zurück (unsortiert; die Reihenfolge stellt der
 * Aufrufer her). `idTag` hält die Schicht-IDs über die Bereiche hinweg
 * eindeutig.
 */
function scheduleZone(
  ...
  // Der Chef läuft nicht über die Nachfrage-Verteilung – seine Stunden sind
  // fest (jeden Tag durchgehend). Nur die übrigen Kräfte teilen sich die
  // gewichtete Tagesnachfrage.
  const demandEmployees = employees.filter((e) => !e.isOwner);
  const totalTargetMin = demandEmployees.reduce((sum, e) => sum + e.targetMinutes, 0);
  const totalWeight = dates.reduce((sum, d) => sum + weightOf(d), 0);

  // Stoßzeit-Sollbesetzung JE BEREICH.
  //
  // In der Küche steht der Chef ohnehin durchgehend; in der Stoßzeit soll eine
  // zweite Kraft dazukommen (minStaff 2). Der Service hat dafür schlicht nicht
  // genug Stunden: die vier Service-Kräfte decken zusammen kaum die
  // durchgehende Einzelbesetzung des Fensters 12:00–22:00. Deshalb ist im
  // Service die harte Vorgabe „immer mindestens EINE Kraft da" (minStaff 1);
  // eine zweite in der Spitze kommt dazu, wo die Stunden reichen (über die
  // Gewichtung und die Reparaturläufe), erzwungen wird sie nicht – sonst
  // reißen anderswo Löcher in die Grundbesetzung. Fehlt sie, weist das
  // Dashboard es als Lücke aus.
  const zone = employees.find((e) => e.workRole)?.workRole;
  const zoneMinStaff = zone === "SERVICE" ? 1 : 2;
  const zonePeaksByWeekday = {} as Record<WeekdayKey, readonly PeakWindow[]>;
  for (const key of Object.keys(PEAK_WINDOWS_BY_WEEKDAY) as WeekdayKey[]) {
    zonePeaksByWeekday[key] = PEAK_WINDOWS_BY_WEEKDAY[key].map((p) => ({
      ...p,
      minStaff: Math.min(p.minStaff, zoneMinStaff),
    }));
  }
  const peaksOfDay = (isoDate: string): readonly PeakWindow[] =>
    zonePeaksByWeekday[effKeyOf(isoDate)];
  ...
  let totalFloor = 0;
  for (const d of dates) {
    const f = peakFloorMinutes(dayOf(d), peaksOfDay(d));
    floors.set(d, f);
    totalFloor += f;
  }
  ...
  const seed = `${idTag}-${employees.map((e) => `${e.id}:${e.targetMinutes}`).join("|")}`;

  const employeesById = new Map(employees.map((e) => [e.id, e] as const));
  // Feste-Schicht-Kräfte (fixedShift) und der Chef laufen NICHT über die
  // normale Blocklogik und die Reparaturläufe – ihr Fenster steht fest; sie
  // werden ganz am Ende belegt.
  const normale = employees.filter((e) => !e.fixedShift && !e.isOwner);
  const festeKraefte = employees.filter((e) => e.fixedShift && !e.isOwner);
  ...
  function attempt(varyLengths: boolean, salt = ""): SchedulerState {
    ...
    const st: SchedulerState = {
      ...
      effKeyOf,
      dayOf,
      peaksOf: (isoDate: string) => peaksOfDay(isoDate),
  ...
  // Ganz zum Schluss die festen Kräfte belegen – unberührt von den
  // Reparaturläufen, damit ihr Fenster fest bleibt: erst etwaige feste
  // Schichten, dann der Chef mit seiner durchgehenden Küchen-Schicht an jedem
  // offenen Tag (Grundbesetzung der Küche).
  placeFixedShiftWorkers(state, festeKraefte);
  placeOwnerShifts(state, owners);

  // Die Reihenfolge stellt der Aufrufer (generateSchedule) über alle Bereiche
  // zusammen her.
  return state.shifts;
}
